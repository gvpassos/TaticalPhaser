...
export function addBotoes(scene) {
    ...
    scene.add.circle(w * 0.7, h * 0.2, 25, 0xff0000)
        .setScrollFactor(0, 0)
        .setInteractive()
        .on('pointerup', ()=>{scene.callMenus['settings'](scene)})
    ...
    if (!scene.Config.touchMove) {
        scene.joyStick = scene.plugins.get('rexVirtualJoystick').add(scene, {
            x: w * 0.12,
            y: h * 0.78,
            radius: w * 0.08,
            base: scene.add.circle(0, 0, 100, 0x888888, 0.4).setStrokeStyle(1.5, 0x250000),
            thumb: scene.add.circle(0, 0, 50, 0x256480, 0.2).setStrokeStyle(1.5, 0xaaaabb),
        })
        scene.joyStick.on('update', () => { scene.player.joystickMove(scene.joyStick) });

        const buttonJoystick = scene.add.circle(w * 0.7, h * 0.75, w * 0.05, 0x256480)
            .setAlpha(0.4)
            .setScrollFactor(0, 0)
            .setStrokeStyle(1.5, 0x250000);

        const button = scene.plugins.get('rexButton').add(buttonJoystick);
        button.on('down', function (button, gameObject, event) {
            scene.player.tecladoMove['x'](1,)
        }, scene);
